...
\usepackage[english,frenchb]{babel}%        rajouter éventuellement english, greek, etc.
...
\begin{figure}
	...
	\hfill
	\begin{minipage}[H]{0.2\linewidth}
		\includegraphics[width=3.3cm]{ris6}
	\end{minipage}
...
%	L'example suivant permet de comprendre les problèmes auxquels s'attaque le calcul des variations. Il s'agit de trouver le chemin le plus court entre deux points. En reformulant ce problème dans la language du calcul des variations, on arrive au fait que c'est une ligne droite. Mais c'était un fait bien connu avant meme l'apparition du calcul des variations et donc notre example ne convaincra personne de l'utilité du calcul des variations.
	...
	\section{Problème de Steiner}\label{sec5}
		...
		\begin{solution*}
			...
			\hfill
			\begin{minipage}[h]{0.49\linewidth}
				\includegraphics[width=4cm]{exp4}
			\end{minipage}
			...
			\begin{figure}[h]
				\begin{center}
					\begin{tikzpicture}
					\draw(0,0)--(1.54,1.2)--(2.43,0.09)--cycle;
					\draw(1.54,1.2)--(2.43,0.09)--(3.01,1.48)--cycle;
					\draw(0,0)--(2.43,0.09)--(1.28,-1.94)--cycle;
					\draw[color=red, fill=red] (1.51,0.76) circle (0.05cm);
					\node[color=red, below left=0.05cm] at (1.51,0.76) {$P$};
					\node[left] at (0,0) {$A$};
					\node[above]  at (1.54,1.2) {$B$};
					\node[right] at (2.43,0.09) {$C$};
					\node[right] at (3.01,1.48) {$A'$};
					\node[below]  at (1.28,-1.94) {$B'$};
					\draw[dashed] (2.36,0.93) circle (0.86cm);
					\draw[dashed] (1.26,-.58) circle (1.37cm);
		...
		\subsection{Problème de l'arbre minimal de Steiner pour $k$ points}
	        ...
	        \subsubsection{La construction de réseau}
	        	...
	        	Nous avons donc $k$ points sur un plan et nous voulons construire le plus petite réseau de routes pour eux. Sipposons qu'un réseau de Steiner a déja été construit. Nous prenons deux sommets de ce réseau, le chemin entre lesquels contient le plus grand nombre d'arêtes. Nous anons indiqué ces sommets par $A$ et $B$. Nous partons du point $B$. Ce sommet n'est pas un sommet supplémentaire, car il est du troisième type. Considérons un sommet $C$ qui est avant du sommet $B$. Il peut être supplémentaire ou initial (figure \ref{point}). 
	        	...
	        	\begin{figure}[h]
	        		...
	        		\node[left] at (-5.9,-0.45) {$a)$};
	        		\node[left] at (-0.9,-0.45) {$b)$};
		    ...
		    \end{figure}
         	...
         	\textbf{2-me méthode} Nous supprimons un couple de sommets. En vertu de la symétrie, il suffit de considerer deux cas: 
         		\begin{enumerate}
         			\item Supprimons $A$ et $C$. Construisons un triangle équilatéral $\Delta ACE$. Les trois points $B, D$ et $E$ se trouvent sur la meme droite (figure \ref{carre2}), donc le réseau de Steiner est constitué de deux  segments $ED$ et $DB$. $ED$ n'intersecte pas le cercle circonscrit du triangle $\Delta ACE$ et donc cette cas ne donne pas un réseau de Steiner.
         				\begin{figure}[h]
         					\begin{center}
         					\begin{tikzpicture}
         					\draw[color=gray, dashed](0,0)--(0,2)--(2,2)--(2,0)--cycle;
         					\draw[fill=black](0,0) circle (0.05cm);
         					\draw[fill=black](0,2) circle (0.05cm);
         					\draw[fill=black](2,2) circle (0.05cm);
         					\draw[fill=black](2,0) circle (0.05cm);
         			...
         			\item Supprimons $C$ et $D$. Construisons un triangle equilatéral $\Delta CDE$. On doit d'abord construire un réseau Steiner pour les points $A, E$ et $B$. Ils se trouvent sur les sommets du triangle (\ref{carre3} $a)$), dont tous les angles sont inférieurs à $120^\circ$. Donc le réseau Steiner est constitué d'un sommet $P$ supplémentaire et de trois arêtes $AP,BP,EP$. L'arête intersect le cercle circonscrit du triangle $\Delta DCE$ au point $M$, ce sera donc notre deuxième sommet supplémentaire lequel on doit connecter les sommets isolés par nous plus tôt. En conséquence, nous obtenons un réseau Steiner comme sur la figure \ref{carre3} $b)$.
         				...
         				\begin{figure}[Ht]
         					...
         					\begin{center}
         					\begin{tikzpicture}
         					\draw[color=gray, dashed](2,0)--(0,0)--(0,2)--(2,2);
         					\draw[fill=black](0,0) circle (0.05cm);
         					\draw[fill=black](0,2) circle (0.05cm);
         					\draw[fill=black](2,2) circle (0.05cm);
         					\draw[fill=black](2,0) circle (0.05cm);
         					...
         					\draw[->] (5.2,1)--(5.8,1);
         					\draw[color=gray, dashed](8,0)--(6,0)--(6,2)--(8,2)--cycle;
         		...
         		\begin{figure}[h]
         			...
         			\begin{minipage}[h]{0.49\linewidth}
         				\begin{center}
         					\includegraphics[width=4cm]{exp5}
         				\end{center}
         			\end{minipage}
...
%		\end{figure}
%		 
%		Ainsi, dans les circuits multi-couches, c'est la métrique rectilinéaire qui utiliséz; le coût d'une liaison (arête) entre deux points du circuit $p_i(x_i;y_i;z_i)$ et $p_j(x_j,y_j,z_j)$ est égal à: $|x_i-x_j|+|y_i-y_j|+|z_i-z_j|$.
...
%	\bibitem{istmath90}  Петров Ю.П. \textit{Из истории вариационного исчисления и теории оптимальных процессов}// Историко-математические исследования.— М.: Наука, 1990, $\No 32/33$ p. 53-73.
%	\bibitem{bois} Бойс Ч. \textit{Мыльные пузыри}-Детиздат ЦК ВЛКСМ, 1937.
	\bibitem{didon} \href{http://math.arizona.edu/~dido/dido-isoperimetrie-histoire.pdf}{http://math.arizona.edu/~dido/dido-isoperimetrie-histoire.pdf}.
	...
	\bibitem{goldstine80} Goldstine H.H. \textit{A history of the calculus of variations from the 17th to the	19th century}-Springer, Berlin, 1980.
	\bibitem{euler} Weisstein, Eric W. "\href{http://mathworld.wolfram.com/Euler-LagrangeDifferentialEquation.html}{Euler-Lagrange Differential Equation}". MathWorld.
	\bibitem{pompeiu} \href{http://mathworld.wolfram.com/PompeiusTheorem.html}{MathWorld's page on Pompeiu's Theorem}
	\bibitem{pente} Yuan, Ya-xiang \href{ftp://lsec.cc.ac.cn/pub/yyx/papers/p0504.pdf}{\textit{"Step-sizes for the gradient method"}}, AMS/IP Studies in Advanced Mathematics. Providence, RI: American Mathematical Society
	\bibitem{wolfe} Wolfe P. \textit{Convergence conditions for ascent methods} //Siam Review- 1969
\end{thebibliography}    
